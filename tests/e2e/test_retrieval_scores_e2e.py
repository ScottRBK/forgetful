...
async def test_scores_aligned_and_non_increasing(mcp_client):
    ...
    try:
        for title, content, keywords in [
            ("Pg scores A", "Kubernetes operators manage custom resources in clusters.", ["k8s", "operator"]),
            ("Pg scores B", "Italian pasta dough rests before rolling thin sheets.", ["pasta", "dough"]),
            ("Pg scores C", "Chess endgames favor rook and pawn structures.", ["chess", "endgame"]),
        ]:
...
async def test_rerank_scores_present_and_ordered(mcp_client):
    ...
    try:
        for title, content, keywords in [
            ("PostgreSQL Database", "PostgreSQL relational ACID database.", ["postgresql", "database"]),
            ("MongoDB Database", "MongoDB document NoSQL database.", ["mongodb", "database"]),
            ("Redis Database", "Redis in-memory cache database fast.", ["redis", "database", "cache"]),
        ]:
            await mcp_client.call_tool(
                "execute_forgetful_tool",
                {
